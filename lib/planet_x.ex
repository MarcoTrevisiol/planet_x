...
defmodule PlanetX do
  ...
  defp add_dwarf_other(sky = %{d: d}) do
    @dwarf_other
    |> Stream.reject(fn [a,b] -> Map.has_key?(sky, add(d, a)) or Map.has_key?(sky, add(d, b)) end)
  ...
  defp add_gas(sky) do
    ...
    |> Stream.reject(fn sk -> Map.has_key?(sk, add(existing_gas,1)) and Map.has_key?(sk, add(existing_gas,17)) end)
  end
